...
interface MovieMatchmakerProps {
  partnerUid: string;
  userWatchlist: MovieWatchlistItem[];
}

export const MovieMatchmaker: React.FC<MovieMatchmakerProps> = ({ partnerUid, userWatchlist }) => {
  ...
  if (matches.length === 0) {
    return (
      <div className="glass-panel" style={{ padding: '40px', textAlign: 'center', maxWidth: '600px', margin: '0 auto' }}>
        ...
        <h3 style={{ margin: '0 0 8px 0' }}>No Movie Matches Found</h3>
        <p style={{ color: 'var(--text-secondary)', margin: 0 }}>
          You and your partner don't have any overlapping movies in your watchlists yet. Start adding some mutual interests!
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
        ...
        <h2 style={{ margin: 0, fontSize: '2rem' }}>Top Movie Compromise Picks</h2>
      </div>
      
      <p style={{ color: 'var(--text-secondary)', marginBottom: '32px' }}>
        These are the movies both of you want to watch, ranked mathematically to find the best compromise for movie night.
      </p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {matches.map((match, idx) => (
          <div key={match.item.id} className="glass-panel" style={{ display: 'flex', gap: '16px', padding: '16px', alignItems: 'center' }}>
            ...
            <img 
              src={`https://image.tmdb.org/t/p/w200${match.item.movie.poster_path}`} 
              alt={match.item.movie.title} 
              style={{ width: '60px', height: '90px', objectFit: 'cover', borderRadius: '8px' }}
            />
            
            <div style={{ flex: 1 }}>
              <h3 style={{ margin: '0 0 4px 0', fontSize: '1.2rem' }}>{match.item.movie.title}</h3>
